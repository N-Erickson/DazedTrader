...
func (sr *StrategyRunner) executeSellSignal(signal Signal) {
	...
	// CRITICAL: Verify we have valid cost basis
	if sr.position.AveragePrice <= 0 {
		msg := fmt.Sprintf("[%s] BLOCKED: Invalid cost basis $%.6f - cannot calculate profit", sr.config.Name, sr.position.AveragePrice)
		log.Println(msg)
		return
	}
	...
	// ABSOLUTE LOSS PREVENTION: NEVER SELL AT A LOSS
	if profitPercent < 0 {
		msg := fmt.Sprintf("[%s] BLOCKED: LOSS PREVENTION - %.2f%% loss (need profit > 0%%)",
			sr.config.Name, profitPercent)
		log.Println(msg)
		return
	}

	// DOUBLE CHECK: Ensure actual dollar profit is positive
	if potentialProfit <= 0 {
		msg := fmt.Sprintf("[%s] BLOCKED: LOSS PREVENTION - $%.2f loss (need dollar profit > $0)",
			sr.config.Name, potentialProfit)
		log.Println(msg)
		return
	}

	// TRIPLE CHECK: Ensure sell price is higher than buy price
	if signal.Price <= sr.position.AveragePrice {
		msg := fmt.Sprintf("[%s] BLOCKED: LOSS PREVENTION - Sell price $%.6f <= Buy price $%.6f",
			sr.config.Name, signal.Price, sr.position.AveragePrice)
		log.Println(msg)
		return
	}
